...
void process_result(void* me, const Mono_Solver& solver)
{
	if (solver.get_terminate()) return;
	CScbDlg* dlg = (CScbDlg*)me;
	CString result = solver.best_result(0);
	if (solver.spaces_scored())
	{
		result = remove_spaces(result);
	}
	if (dlg->preserve_original_format())
	{
		result = Mono_Solver::reformat(result, dlg->m_ciphertext);
	}
	int keys = solver.keys_checked();
	if (result != dlg->m_plaintext)
	{
	   dlg->m_plaintext = result;
	   dlg->m_keys_checked = solver.keys_checked();
	   dlg->m_key = solver.best_key();
	   char tmp[100];
	   sprintf(tmp, "%d", solver.score());
	   dlg->m_score = tmp;
       dlg->SendMessage(M_SOLVERCALLBACK, FALSE, 0);
	}
	else if (keys - ((CScbDlg*)me)->m_keys_checked > 100000)
	{
	   dlg->m_keys_checked = solver.keys_checked();
       dlg->SendMessage(M_SOLVERCALLBACK1, FALSE, 0);	
	}
}
...
void CScbDlg::OnSolve() 
{
	...
	solver.set_verbose();
	solver.set_user_callback(this, process_result);
...
static void adjust_cipher_text(char* _ciphertext)
{
	return;
		  int* spaces = new int [ strlen(_ciphertext) + 1 ];
		  char* c = _ciphertext;
		  // Find spaces
		  int sc = 0;
		  int index = 0;
		  ...
		  {
		     index = 0;
		     while (index == 0 || index == strlen(_ciphertext) - 1)
			 {
		        int si = rand() % sc;
		        index = spaces[si];
			 }
			 int inc = rand() % 2;
			 if (inc == 0) inc = -1;
		     char tmp = _ciphertext[index + inc];
		     _ciphertext[index + inc] = ' ';
		     _ciphertext[index] = tmp;
		  }
}
